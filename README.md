...
* 유지보수
   ...
   - api 불러오는 부분 구분하여 로딩 속도 줄이기
      ...
      . weekly => 모든 강습을 불러와서 서버시간 필터링 / api 자체에서 필터링해서 가져오고 있음.
         ...
         -> 클라이언트 -> next 서버 -> 노션 api 순서로 진행하는 방법으로 개선
         -> 수상자 소팅은 클라이언트에서 진행 / 리스트 내 서칭은 api로 호출
      ...
      . 🔥 100개 이상의 데이터 호출 시 페이지네이션을 통해서 추가 요청하는 형태 작업 필요
         ...
         종료된 행사 대회 워크샵 강습은 별도로 저장하여 [종료] 항목으로 리스트 나열
         -> 종료되면 그때마다 매번 npm run update 해줘야함... 이거 어떻게 할지 고민 필요
