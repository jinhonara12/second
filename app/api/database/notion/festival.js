...
import styles from "../../../(recruitment)/festival-recruitment/[id]/[slug]/page.module.scss"
...
export default async function festival(id) {
    ...
    const textBlocks = data.filter((item) =>
        ["heading_3", "bulleted_list_item", "numbered_list_item", "callout"].includes(item.type)
    )
    ...
    const pairedTextBlocks = textBlocks.reduce((acc, currentItem, i, arr) => {
        if (currentItem.type === "heading_3") {
            ...
            while (
                nextItem &&
                (nextItem.type === "bulleted_list_item" ||
                    nextItem.type === "numbered_list_item" ||
                    nextItem.type === "callout")
            ) {
                        ...
                        <div className={styles.detail_text_box}>
                            {item.text
                                ? item.text.map((text, index) => {
                                      return (
                                          <span className={styles.detail_text} key={index}>
                                              {text}
                                          </span>
                                      )
                                  })
                                : item.href.map((href, index) => {
                                      ...
                                      return (
                                          <a className={styles.detail_link} href={link} key={index} target="_blank">
                                              {title} <img src="/icons/link_24px.png" />
                                          </a>
                                      )
                                  })}
                        </div>
