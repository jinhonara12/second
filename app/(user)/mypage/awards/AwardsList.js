    ...
    return (
        <div className={styles.awards}>
            ...
            {awards.length === 0 ? (
                ...
                <ul className={styles.awards_list}>
                    <li className={`${styles.awards_list__head} ${styles.pc}`}>
                        <span data-sort="name" onClick={sorting}>
                            대회명
                            <img src="/icons/sorting.svg" />{" "}
                        </span>
                        <span data-sort="start_date" onClick={sorting}>
                            대회일자
                            <img src="/icons/sorting.svg" />{" "}
                        </span>
                        <span data-sort="level" onClick={sorting}>
                            부문-1
                            <img src="/icons/sorting.svg" />{" "}
                        </span>
                        <span data-sort="division" onClick={sorting}>
                            부문-2
                            <img src="/icons/sorting.svg" />{" "}
                        </span>
                        <span data-sort="result" onClick={sorting}>
                            결과
                            <img src="/icons/sorting.svg" />{" "}
                        </span>
                        <span>수정</span>
                    ...
                    {awards.map((award, index) => (
                        <li key={index} className={`${styles.awards_list__item} `}>
                            <div className={styles.pc}>
                                ...
                                <span
                                    onClick={() => clickDelete(award.page_id)}
                                    className={`${styles.delete} ${
                                        deletingIds.includes(award.page_id) ? styles.updating : ""
                                    }`}
                                >
